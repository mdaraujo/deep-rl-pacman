...
if __name__ == "__main__":
    ...
    plot_line(x, train_returns, '{} Training Episodes Return | Total Episodes: {}'.format(
        agent_name, len(train_returns)),
    ...
    plot_line(x[moving_n-1:], moving_returns,
              '{} Training Episodes Return Moving Average | Total Episodes: {}'.format(
                  agent_name, len(train_returns)),
    ...
    if 'score' in train_results:
        ...
        plot_line(x, train_scores, '{} Training Episodes Score | Total Episodes: {}'.format(
            agent_name, len(train_scores)),
        ...
        plot_line(x[moving_n-1:], moving_scores,
                  '{} Training Episodes Score Moving Average | Total Episodes: {}'.format(
                      agent_name, len(train_scores)),
    ...
    if 'ghosts' in train_results:
        ...
        plot_line(x, train_ghosts, '{} Training Episodes N Ghosts | Total Episodes: {}'.format(
            agent_name, len(train_ghosts)),
        ...
        plot_line(x[moving_n-1:], moving_ghosts,
                  '{} Training Episodes Ghosts Moving Average | Total Episodes: {}'.format(
                      agent_name, len(train_ghosts)),
